...
static
device_platform_t create_device();

static
cl_program build_program(cl_context context, cl_device_id device, const char *path);

static
float *generate_net(size_t wdth);

static
double now_ms();

static
cl_mem configure_shared_data(cl_context context, GLuint &pbo, size_t size);

static
void do_work() {
	
	cl_int err;
	cl_event dim_x_event, dim_xy_event;
	size_t wrk_unit_wdth = static_cast<size_t>(_wdth);
	size_t wrk_units[] = { wrk_unit_wdth, static_cast<size_t>(_hght) };
...
float *generate_net(size_t wdth){
  ...
  for(x = 0, y = 0, i = 0; i < net_len; x+=n, i+=4) {
    ...
    printf("[%3lu %3d %4.1f] ", x%wdth, y, output[i+2]);
  }
  printf("\n");
